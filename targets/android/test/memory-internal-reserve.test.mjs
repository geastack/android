...
const platform = readFileSync(new URL('../native/android_memory.cpp', import.meta.url), 'utf8')

assert.match(platform, /Memory::reserveInternalDma\(std::size_t\)/, 'android_memory.cpp must define reserveInternalDma')
assert.match(platform, /Memory::releaseInternalDma\(void \*\)/, 'android_memory.cpp must define releaseInternalDma')
